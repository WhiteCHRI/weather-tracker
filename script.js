...
function saveEntry() {
  ...
  document.getElementById("status").innerText = "✅ Entry saved locally!";
...
function syncToGoogleSheets(entry) {
  ...
  .then(response => {
    if (response.ok) {
      document.getElementById("status").innerText = "✅ Entry saved and synced to Google Sheets!";
    } else {
      document.getElementById("status").innerText = "⚠️ Saved locally, but failed to sync.";
    }
  })
  .catch(error => {
    document.getElementById("status").innerText = "⚠️ Saved locally, but sync error occurred.";
